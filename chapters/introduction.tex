...
{The factorial of a number $n$ is denoted as $n!$. The math behind a factorial is $n! = 1 * 2 * 3 * ... * (n-1) * n$.

In other words, the factorial of $n$ is the product of every number between $1$ and $n$ inclusive.

The exception to this rule is when $n = 0$, where $0! = 1$. This is the same as $1!$ where $1! = 1$.
...
\inputcpp{code/apples_and_oranges_solution.cpp}
